...
export const getCurrencySymbol = (currency) => {
  ...
  return new Intl.NumberFormat(CURRENCY_CONFIG[code].locale, {
    style: 'currency',
    currency: code,
...
export const formatCurrencyAmount = (amountEur, currency = 'EUR', options = {}) => {
  const code = normalizeCurrency(currency);
...
export const formatCurrencyNumber = (amountEur, currency = 'EUR', options = {}) => {
  const code = normalizeCurrency(currency);
  const locale = CURRENCY_CONFIG[code].locale;
  const amount = convertFromEur(amountEur, code);
  const {
