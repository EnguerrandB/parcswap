...
const fetchPublicParkingsCallable = httpsCallable(
  functions,
  "fetchPublicParkings",
);
...
const getForcedPublicParkingCityId = () => {
  ...
  try {
    ...
    const value = String(url.searchParams.get("parkingTestCity") || "")
      .trim()
      .toLowerCase();
...
export const fetchNearbyPublicParkings = async ({
  lng,
  lat,
  radiusMeters = 2000,
  limit = 20,
  ...
  const forcedCenter = forcedCityId
    ? PUBLIC_PARKING_TEST_CITIES[forcedCityId]
    : null;
  ...
  const parkings = Array.isArray(response?.data?.parkings)
